...
export default function Topbar() {
  ...
  const [profileOpen, setProfileOpen] = useState(false); const [loggingOut, setLoggingOut] = useState(false);
  ...
  return <header className="topbar"><button className="icon-btn mobile-menu" onClick={() => setMobileOpen(true)} aria-label="Open navigation"><i className="bi bi-list"/></button><WorkspaceSearch/><div className="topbar-actions"><button className="icon-btn" onClick={toggleTheme} aria-label={`Switch to ${isDark ? 'light' : 'dark'} mode`}><i className={`bi bi-${isDark ? 'sun' : 'moon-stars'}`}/></button><div className="profile-wrap"><button className="profile-button" onClick={() => setProfileOpen(!profileOpen)} aria-expanded={profileOpen} aria-label="Account menu"><span>{initialsOf(user?.full_name)}</span><i className="bi bi-chevron-down"/></button>{profileOpen && <div className="profile-menu"><strong>{user?.full_name || 'ShelfSense workspace'}</strong>{user?.email && <small className="profile-email">{user.email}</small>}<button onClick={() => { setProfileOpen(false); navigate('/settings'); }}>Account settings</button><button className="profile-logout" onClick={handleLogout} disabled={loggingOut}><i className="bi bi-box-arrow-right"/>{loggingOut ? 'Logging out…' : 'Log out'}</button></div>}</div></div></header>;
}
